...
HT_tuning_semillerio <- function( pinputexps, semillerio, bo_iteraciones, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    boosting = "dart", # Cambiado a DART para habilitar el dropout
    objective = "binary",
    metric = "custom",
    first_metric_only = TRUE,
    boost_from_average = TRUE,
    ...
    max_bin = 31L, # lo dejo fijo, no participa en la BO
    
    num_iterations = 9999L, # un número muy grande, se usa early stopping
    early_stopping_base = 200L, # Early stopping según métrica de validación
    ...
    # Parámetros específicos de DART
    drop_rate = 0.1, # Proporción de árboles eliminados en cada iteración
    max_drop = 50L, # Máximo número de árboles eliminados, <=0 significa sin límite
    skip_drop = 0.5, # Probabilidad de omitir el dropout en una iteración (0.0 - 1.0)
    xgboost_dart_mode = FALSE, # Usar el modo de DART en XGBoost (opcional)
    
    # Parámetro opcional para garantizar diversidad adicional
    uniform_drop = FALSE, # Si TRUE, aplica dropout uniforme sobre los árboles existentes
    
    # Parte variable
    learning_rate = c(0.05, 0.3), # Tasa de aprendizaje ajustada para DART (menores valores tienden a ser mejores)
    feature_fraction = c(0.05, 0.95), # Selección de características
...
wf_SEMI_sep_Final <- function( pnombrewf )
{
  ...
  ht <- HT_tuning_semillerio(
    ...
    bo_iteraciones = 80  # iteraciones inteligentes, apenas 10
  )


  fm <- FM_final_models_lightgbm_semillerio( 
    c(ht, ts9), # los inputs
    ranks = 1:10, # 1 = el mejor de la bayesian optimization
