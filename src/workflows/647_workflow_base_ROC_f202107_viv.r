...
EV_evaluate_conclase_gan <- function( pinputexps )
{
  ...
  # para graficar
  param_local$graficar$envios_desde <-   7000L
  param_local$graficar$envios_hasta <-  18000L
...
wf_julio_AUC <- function( pnombrewf )
{
  ...
  fm <- FM_final_models_lightgbm(
    c(ht, ts7),   # inputs del experimento
    ranks=c(1),  # las posiciones de la Bayesian Optimization
    qsemillas=30  # la cantidad de modelos que genero con identicos hiperparam cambiando solo la semilla
  )
