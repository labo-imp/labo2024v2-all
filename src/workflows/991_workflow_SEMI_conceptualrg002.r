...
envg$EXPENV$exp_dir <- "~/buckets/b1/expw-SEMIb/"
envg$EXPENV$wf_dir <- "~/buckets/b1/flow-SEMIb/"
...
HT_tuning_semillerio <- function( pinputexps, semillerio, bo_iteraciones, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    #####
    # num_leaves = c(20L, 200L, "integer"),  # AGREGADA BORRAR
    ...
    # learning_rate = c(0.01, 0.1), # original
    # feature_fraction = c( 0.05, 0.95 ), # original
