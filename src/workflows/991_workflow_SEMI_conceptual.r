...
DR_drifting_base <- function( pinputexps, metodo)
{
  ...
  param_local$metodo <- "deflacion"  # Modificado según PDF
  param_local$semilla <- NULL  # no usa semilla, es deterministico
...
FEhist_base <- function( pinputexps)
{
  ...
  param_local$Tendencias1$minimo <- TRUE  # Modificado según PDF
  param_local$Tendencias1$maximo <- TRUE  # Modificado según PDF
  param_local$Tendencias1$promedio <- TRUE  # Modificado según PDF
...
HT_tuning_semillerio <- function( pinputexps, semillerio, bo_iteraciones, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    lambda_l1 = c(150, 300),  # Modificado según PDF
    lambda_l2 = c(250, 400),  # Modificado según PDF
  ...
  param_local$bo_iteraciones <- 50  # 50 conservado según tu estructura
...
KA_evaluate_kaggle_semillerio <- function( pinputexps )
{
  ...
  param_local$envios_salto <-   20L  # Ajustado según PDF
...
wf_SEMI_sep <- function( pnombrewf )
{
  ...
  FEintra_manual_base()
  DR_drifting_base(metodo="deflacion")  # Modificado
  FEhist_base()
  ...
  ht <- HT_tuning_semillerio(
    semillerio = 50, 
    bo_iteraciones = 50
  )  # 50 iteraciones, ajustes conservados
