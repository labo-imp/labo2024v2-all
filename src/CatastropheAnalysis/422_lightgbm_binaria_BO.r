...
require("DiceKriging")
require("mlrMBO")
require("progress") # Para la barra de progreso
...
pb <- progress_bar$new(
  format = "  [:bar] :percent en :elapsed",
  total = PARAM$hyperparametertuning$iteraciones, clear = FALSE, width = 60
)
...
hs <- makeParamSet(
  ...
  makeIntegerParam("envios", lower = 1000L, upper = 7000L),
  makeIntegerParam("min_gain_to_split", lower = 0.01, upper = 0.3),
  makeIntegerParam("lambda_l1", lower = 0, upper = 10),
  makeIntegerParam("lambda_l2", lower = 0, upper = 10),
  makeIntegerParam("max_depth", lower = 3, upper = 6),
  makeIntegerParam("early_stopping_rounds", lower = 6, upper = 50)
)
