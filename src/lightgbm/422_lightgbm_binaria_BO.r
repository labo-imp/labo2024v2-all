...
hs <- makeParamSet(
  # Hiperparámetros que ya estaban definidos en el script
  makeNumericParam("learning_rate", lower = 0.01, upper = 0.2),           # Tasa de aprendizaje
  makeIntegerParam("num_leaves", lower = 16L, upper = 512L),              # Número de hojas del árbol
  makeNumericParam("feature_fraction", lower = 0.5, upper = 0.9),         # Fracción de características
  makeIntegerParam("min_data_in_leaf", lower = 10L, upper = 4000L),       # Mínima cantidad de datos en una hoja
  makeIntegerParam("envios", lower = 1000L, upper = 3000L),              # Cantidad de envíos
  
  # Hiperparámetros sugeridos para agregar
  makeIntegerParam("max_depth", lower = 3L, upper = 15L),                 # Profundidad máxima de los árboles
  makeNumericParam("bagging_fraction", lower = 0.6, upper = 1.0),         # Fracción de datos para el bagging
  makeIntegerParam("bagging_freq", lower = 1L, upper = 7L),               # Frecuencia del bagging
  makeNumericParam("lambda_l1", lower = 0.0, upper = 1000.0),                # Regularización L1
  makeNumericParam("lambda_l2", lower = 0.0, upper = 1000.0),                # Regularización L2
  makeNumericParam("min_gain_to_split", lower = 0.01, upper = 3.0),       # Ganancia mínima para dividir un nodo
  makeNumericParam("path_smooth", lower = 0.0, upper = 1.0)              # Suavizado de splits del árbol
)
