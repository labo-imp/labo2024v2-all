...
hs <- makeParamSet(
  ...
  makeNumericParam("feature_fraction", lower = 0.1, upper = 1.0),
  makeIntegerParam("min_data_in_leaf", lower = 10L, upper = 1000L),
  ...
  makeIntegerParam("max_bin", lower = 50L, upper = 255L),
  makeNumericParam("scale_pos_weight", lower = 1.0, upper = 10.0),
...
EstimarGanancia_lightgbm <- function(x) {
  ...
  # Parámetros básicos que no se optimizan
  param_basicos <- list(
    objective = "binary",
    metric = "custom",
  ...
  # el parametro early_stopping_rounds, que depende del learning_rate
  param_variable <- list(
    early_stopping_rounds = as.integer(50 + 5 / x$learning_rate)
  )
  
  # Parametros de lightgbm que vienen en x
  param_lightgbm <- list(
  ...
  # Junto todos los parámetros
  param_completo <- c(param_basicos, param_variable, param_lightgbm)
  ...
  xx$envios <- x$envios  # mantenemos envios en el log como estaba originalmente
