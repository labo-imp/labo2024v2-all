...
envg$EXPENV$exp_dir <- "~/buckets/b1/expw-07/"
envg$EXPENV$wf_dir <- "~/buckets/b1/flow-07/"
...
# 
# prueba <- read_csv("~/buckets/b1/datasets/conceptual_competencia_2024.csv.gz")
# 
# table(prueba$foto_mes, prueba$clase_ternaria)
# 
# # Calcular la cantidad de bajas por `foto_mes` y `clase_ternaria`
# df_counts <- prueba %>%
#   filter(clase_ternaria %in% c("BAJA+1", "BAJA+2")) %>% 
#   group_by(foto_mes, clase_ternaria) %>%
#   summarise(count = n(), .groups = 'drop')
# 
# # Calcular el promedio y percentiles de las cantidades mensuales por `clase_ternaria`
# summary_stats <- df_counts %>%
#   group_by(clase_ternaria) %>%
#   summarise(promedio = mean(count),
#             perc_25 = quantile(count, 0.25),
#             perc_75 = quantile(count, 0.75))
# summary_stats
# 
# prueba %>%
#   filter(clase_ternaria %in% c("BAJA+1", "BAJA+2")) %>% 
#   mutate(foto_mes = as.Date(paste0(foto_mes, "01"), format = "%Y%m%d")) %>% 
#   ggplot(aes(x = foto_mes)) +
#   geom_bar() +
#   facet_wrap(~ clase_ternaria, scales = "free_y") +
#   geom_hline(data = summary_stats, aes(yintercept = promedio), color = "red", linetype = "solid", size = 1) +
#   geom_hline(data = summary_stats, aes(yintercept = perc_25), color = "blue", linetype = "dashed", size = 0.8) +
#   geom_hline(data = summary_stats, aes(yintercept = perc_75), color = "blue", linetype = "dashed", size = 0.8) +
#   scale_x_date(date_labels = "%b %Y", date_breaks = "1 month") +
#   labs(title = "Cantidad de Observaciones por Mes y Clase Ternaria", 
#        x = "Mes y Año", 
